...
def generate_synthetic_data(tags, num_samples, min_length=5, max_length=15):
    ...
    def generate_sequence(length):
        ...
        if secrets.SystemRandom().random() < 0.7:  # 70% chance to add an entity
            # Choose random tag
            tag = secrets.choice(tags)
            # Choose random start position
            start = secrets.SystemRandom().randint(0, length - 2)
            # Choose random length (1 or 2 tokens)
            entity_length = secrets.SystemRandom().randint(1, 2)
    ...
    def generate_prediction(true_sequence):
        ...
        for i in range(len(pred_sequence)):
            if secrets.SystemRandom().random() < 0.2:  # 20% chance to modify each token
                if pred_sequence[i] == 'O':
                    ...
                    elif secrets.SystemRandom().random() < 0.3:
    ...
    for _ in range(num_samples):
        length = secrets.SystemRandom().randint(min_length, max_length)
